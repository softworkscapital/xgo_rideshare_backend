...
crudsObj.postUser = (user) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    pool.query(
      "INSERT INTO users(userid, username, password, role, email, notify, activesession, addproperty, editproperty, approverequests, delivery, status, employee_id, company_id, branch_id, sync_status, last_logged_account, driver_id, customerid, otp, signed_up_on, last_logged_in, last_activity_date_time, last_fin_activity_date_time, referral_code, reference_payment_status, referred_by, referred_by_me_count, customer_adoption_type, my_current_wait_branch, my_job_title_id, reporting_to_job_title_id, reporting_to_user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [
        user.userId,
        user.username,
        user.password,
        ...
        user.my_current_wait_branch,
        user.my_job_title_id,
        user.reporting_to_job_title_id,
        user.reporting_to_user_id,
      ],
      (err, result) => {
        if (err) {
        ...
        return resolve({ userid: user.userid }); // Resolve with the userid
      }
    );
  });
};
...
crudsObj.postUser2 = (
  ...
  return new Promise((resolve, reject) => {
    pool.query(
      "INSERT INTO users(company_id,branch_id,username,password,role,category,email,notify,activesession,addproperty,editproperty,approverequests,delivery,status,client_profile_id, OTP,signed_up_on, reffered_by_me_count, customer_adoption_type,  my_current_wait_branch, my_job_title_id, reporting_to_job_title_id,  reporting_to_user_id) VALUES (?, ?, ?, ?, ?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?, ?)",
      [
        company_id,
        branch_id,
        username,
...
crudsObj.getFullUserDetailsWithJobTitle = (userId) => {
  return new Promise((resolve, reject) => {
    const query = `
      SELECT u.*, jt.job_title_name, jt.job_title_grade, jt.job_title_department 
      FROM users u 
      LEFT JOIN job_title jt ON u.my_job_title_id = jt.job_title_id 
      WHERE u.userid = ?`;
...
crudsObj.getFullUserDetailsWithSupervisorTitle = (userId) => {
  return new Promise((resolve, reject) => {
    const query = `
      SELECT u.*, jt.* 
      FROM users u 
      LEFT JOIN job_title jt ON u.reporting_to_job_title_id = jt.job_title_id 
      WHERE u.userid = ?`;
...
crudsObj.updateUser = (userid, updatedValues) => {
  return new Promise((resolve, reject) => {
    ...
    const values = [
      ...
      updatedValues.customer_adoption_type,
      updatedValues.my_current_wait_branch,
      updatedValues.my_job_title_id,
      updatedValues.reporting_to_job_title_id,
      updatedValues.reporting_to_user_id,
      userid,
    ];
...
//-----------------all nee to be transfered  ------------------
crudsObj.getUsersByJobTitleId = (jobTitleId) => {
  return new Promise((resolve, reject) => {
    const query = `
      SELECT * FROM users 
      WHERE my_job_title_id = ?;
    `;

    pool.query(query, [jobTitleId], (err, results) => {
      if (err) {
        return reject(err);
      }
      return resolve(results); // Return the list of users
    });
  });
};
crudsObj.terminateEmployee = (userid) => {
  return new Promise((resolve, reject) => {
    pool.query(
      `UPDATE users SET status = 'terminated' WHERE userid = ?`,
      [userid],
      (err, result) => {
        if (err) {
          return reject(err);
...
crudsObj.getEmployeesByRoles = () => {
  return new Promise((resolve, reject) => {
    const roles = ["Human Resources", "Admin", "Executive", "Support"];
    pool.query(
...
crudsObj.getJobTitlesWithUserCounts = () => {
  return new Promise((resolve, reject) => {
    const query = `
      SELECT jt.job_title_id, jt.job_title_name, 
             COUNT(u.userid) AS user_count 
      FROM job_title jt 
      LEFT JOIN users u ON jt.job_title_id = u.my_job_title_id 
      GROUP BY jt.job_title_id, jt.job_title_name
    `;
